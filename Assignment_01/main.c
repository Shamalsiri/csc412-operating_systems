...
Node* string2list(char* list, int length, float scale);
...
void* search(void* sData)
{
	...
	while(head != NULL)
	{
		...
		{
			Node* replacementList = string2list(replacementPattern, start->length, scale);
			//Call replacement via createthreads with start and end.
			printf("%s\n", listPattern);
			printf("Pattern Matches\n");
			struct replacementRules* replacementR = malloc(sizeof(struct replacementRules*));
			replacementR->replacementList = replacementList;
